        ...
        <div class="card-body">
            @if($enabled)
                <div class="alert alert-success"
                    style="background: var(--success-bg, #ecfdf5); color: var(--success-text, #065f46); padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem;">
                ...
                <form action="{{ route('admin.settings.security.disable') }}" method="POST"
                    ...
                    <button type="submit" class="btn btn-danger"
                        style="background: var(--error-text, #ef4444); color: white; padding: 0.5rem 1rem; border-radius: 0.5rem; border:none; cursor:pointer;">
                        Disable 2FA
                    </button>
                </form>
            @else
                <div class="grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                    ...
                    <div class="method-card"
                        style="border: 1px solid var(--border-color, #e5e7eb); padding: 1.5rem; border-radius: 0.5rem;">
                        <div style="margin-bottom: 1rem;">
                            <span
                                style="background: var(--info-bg, #eff6ff); color: var(--info-text, #1d4ed8); padding: 0.25rem 0.5rem; border-radius: 9999px; font-size: 0.75rem; font-weight: bold;">Recommended</span>
                        </div>
                        ...
                        <button onclick="startSetup('app')" class="btn"
                            style="width: 100%; background: var(--primary, #3b82f6); color: white; padding: 0.5rem; border-radius: 0.5rem; border: none; cursor: pointer;">
                            Enable via App
                        </button>
                    </div>
                    ...
                    <div class="method-card"
                        ...
                        <button onclick="startSetup('email')" class="btn"
                            style="width: 100%; background: var(--card-bg, white); border: 1px solid var(--border-color, #d1d5db); color: var(--text-main, #374151); padding: 0.5rem; border-radius: 0.5rem; cursor: pointer;">
                            Enable via Email
                        </button>
                    </div>
                </div>
                ...
                <!-- Setup Modal -->
                <div id="setupArea"
                    style="display: none; margin-top: 2rem; border-top: 1px solid var(--border-color, #e5e7eb); padding-top: 2rem;">
                    ...
                    <div id="qrArea" style="display: none; margin-bottom: 1.5rem; text-align: center;">
                        <p class="text-sm text-gray-500" style="margin-bottom: 1rem;">Scan this QR code with your authenticator
                            app:</p>
                        <div id="qrImage"
                            style="background: var(--card-bg, white); padding: 1rem; display: inline-block; border: 1px solid var(--border-color, #e5e7eb); border-radius: 0.5rem;">
                        </div>
                    ...
                    <form id="confirmForm" action="{{ route('admin.settings.security.confirm') }}" method="POST">
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            <input type="text" name="code" required placeholder="123456"
                                style="flex: 1; padding: 0.5rem; border: 1px solid var(--border-color, #d1d5db); border-radius: 0.5rem; background: var(--bg-color, white); color: var(--text-main, black);">
                            <button type="submit" class="btn"
                                style="background: var(--success-text, #10b981); color: white; padding: 0.5rem 1.5rem; border: none; border-radius: 0.5rem; cursor: pointer;">
                                Verify & Enable
                            </button>
                        </div>
                    </form>
