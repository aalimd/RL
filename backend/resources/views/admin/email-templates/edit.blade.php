            ...
            <form action="{{ route('admin.email-templates.update', $template->id) }}" method="POST">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <input type="text" name="subject" class="form-input" value="{{ old('subject', $template->subject) }}"
                        required
                        style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color, #d1d5db); border-radius: 0.375rem; background: var(--bg-color, white); color: var(--text-main, black);">
                </div>
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <div style="margin-bottom: 0.5rem;">
                        <span style="font-size: 0.875rem; color: var(--text-muted, #6b7280);">Available Variables:</span>
                        @foreach($template->variables ?? [] as $var)
                            <code
                                style="background: var(--card-bg, #f3f4f6); padding: 2px 4px; border-radius: 4px; color: var(--text-muted, #4b5563); margin-right: 5px; cursor: pointer;"
                                onclick="insertVar('{!! '{' . $var . '}' !!}')">{!! '{' . $var . '}' !!}</code>
                        @endforeach
                    </div>
                    <textarea name="body" id="bodyEditor" class="form-textarea" rows="15" required
                        style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color, #d1d5db); border-radius: 0.375rem; font-family: monospace; background: var(--bg-color, white); color: var(--text-main, black); color-scheme: dark;">{{ old('body', $template->body) }}</textarea>
                </div>
