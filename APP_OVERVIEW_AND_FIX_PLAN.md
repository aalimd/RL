...
- Summary: Made the browser-faithful Chrome/Browserless PDF renderer the official student PDF path, changed the approved tracking CTA to open a review viewer before download, added a dedicated PDF viewer page with only Back and Download actions, intentionally excluded student cloud-upload options, added a render-readiness event for Browserless, validated PDF bytes, and blocked official export if the rendered PDF exceeds one A4 page.
