...
| `LET-01` | Student Document UX / Letter Presentation | P1 | `done` | Codex | Redesign the student-facing letter experience around a calm official-document viewer: one primary official action, minimal browser chrome, no distracting print instructions, professional A4 preview, and visual language that feels institutional rather than web-app decorative. Shipped a dedicated student review viewer that embeds the official PDF, keeps Download PDF as the main action, keeps Back to Tracking as the only secondary action, and intentionally excludes student cloud-upload options. | `STU-06`, `VIS-01` | Approved students see one clear official letter experience, with a clean A4 preview, a primary “Download official PDF” or equivalent action, secondary print/back controls, and no UI elements appearing in print/export output. |
| `LET-02` | Letter Design System / Professional Standards | P1 | `todo` | `unassigned` | Create a strict recommendation-letter design baseline for English and Arabic: A4-safe margins, conservative typography, predictable header/body/signature/footer zones, restrained institutional color use, QR/verification placement rules, and content-length guidance that admins can understand before approval. | `LET-01`, `ADM-02` | New and edited templates follow one professional letter baseline, Arabic and English output use appropriate readable fonts and direction rules, decorative options cannot overpower the document, and admins can see clear guidance for content that risks overflow. |
| `PDF-01` | Student Document Delivery / Canonical Output | P1 | `done` | Codex | Replace browser-dependent student saving with a canonical server-generated official artifact that uses the browser-faithful renderer, not the current low-fidelity Dompdf path. Shipped the student PDF route through the Chrome/Browserless renderer, changed the approved tracking CTA to the review viewer first, and kept the direct PDF route stable for inline viewing and download. | `LET-01`, `PDF-03` | Mobile, desktop, Safari, Chrome, Brave, and Edge users receive the same official PDF bytes from the server; the PDF visually matches the approved browser-rendered letter in English and Arabic; and the browser print dialog is no longer the only reliable way to save the official letter. |
| `PDF-02` | Letter Rendering / One-Page A4 Fit Engine | P1 | `done` | Codex | Rebuild the one-page guarantee around the actual browser-faithful PDF renderer and page count, not responsive CSS scale or Dompdf-only checks. Shipped real PDF validation, page-count detection, and a hard stop that blocks official student/admin export when the rendered PDF exceeds one A4 page instead of silently shipping a broken document. Removed Browserless dependency on the fragile custom `letter-fit-ready` wait event after production showed it can time out. | `PDF-01`, `LET-02` | Every official exported letter is exactly one A4 page when content is within safe limits; oversized content is caught before student delivery; no text, Arabic glyphs, footer, signature, stamp, QR, watermark, or border is clipped or pushed outside the frame. |
...
| `ADM-03` | Admin Workflow / Letter Fit Diagnostics | P1 | `todo` | `unassigned` | Add admin-side fit and quality diagnostics based on the same canonical browser-faithful output students receive. Show whether the letter fits one A4 page, whether it was auto-fitted, and what section is causing overflow before the admin approves or exports. | `PDF-02`, `LET-02`, `ADM-01` | Admins can preview the exact student-facing output, see one-page status and overflow source, receive clear shortening guidance, and avoid approving letters that would export as clipped, unprofessional, or multi-page documents. |
...
### 2026-04-25 - Task Update

- Task ID: `LET-01`, `PDF-01`, `PDF-02`
- Area: Student Document UX / Official PDF Delivery
- Owner: Codex
- Status: `done`
- Files: `backend/app/Services/BrowserLetterPdfService.php`, `backend/app/Http/Controllers/PageController.php`, `backend/resources/views/public/letter-viewer.blade.php`, `backend/resources/views/public/letter.blade.php`, `backend/resources/views/public/tracking.blade.php`, `backend/tests/Feature/SecurityRegressionTest.php`
- Summary: Made the browser-faithful Chrome/Browserless PDF renderer the official student PDF path, changed the approved tracking CTA to open a review viewer before download, added a dedicated PDF viewer page with only Back and Download actions, intentionally excluded student cloud-upload options, validated PDF bytes, blocked official export if the rendered PDF exceeds one A4 page, and removed the fragile Browserless custom-event wait after production timeout errors.
- Testing: `php -l backend/app/Services/BrowserLetterPdfService.php`; `php -l backend/app/Http/Controllers/PageController.php`; `php -l backend/tests/Feature/SecurityRegressionTest.php`; `php artisan test --filter=SecurityRegressionTest --do-not-cache-result`; `php artisan test --do-not-cache-result`
- Result: Students now review the same server-generated official PDF before downloading it, the old browser-print workflow is no longer the primary student delivery method, and oversized rendered PDFs fail safely for admin review instead of being silently shipped.
